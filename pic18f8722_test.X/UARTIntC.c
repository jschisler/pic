...
unsigned char UARTIntPutChar( unsigned char chCharData )
{
	...
	if ( vUARTIntTxBufDataCnt == TX_BUFFER_SIZE ) vUARTIntStatus.UARTIntTxBufferFull = 1;
	vUARTIntTxBufWrPtr++;
	if ( vUARTIntTxBufWrPtr == TX_BUFFER_SIZE ) vUARTIntTxBufWrPtr = 0;
	PIE1bits.TXIE = 1;
...
void UARTIntISR( void )
{
	...
	if ( PIR1bits.TXIF & PIE1bits.TXIE )
	{
		if ( !vUARTIntStatus.UARTIntTxBufferEmpty )
		{
			...
			if ( vUARTIntStatus.UARTIntTxBufferFull ) vUARTIntStatus.UARTIntTxBufferFull = 0;
			vUARTIntTxBufDataCnt--;
			if ( vUARTIntTxBufDataCnt == 0 ) vUARTIntStatus.UARTIntTxBufferEmpty = 1;
			vUARTIntTxBufRdPtr++;
			if ( vUARTIntTxBufRdPtr == TX_BUFFER_SIZE ) vUARTIntTxBufRdPtr = 0;
		}
		else
		{
			PIE1bits.TXIE = 0;
...
#if RXON
		{
			...
			else if ( !vUARTIntStatus.UARTIntRxBufferFull )
			{
				...
				vUARTIntRxBufDataCnt++;
				if ( vUARTIntRxBufDataCnt == RX_BUFFER_SIZE ) vUARTIntStatus.UARTIntRxBufferFull = 1;
				vUARTIntRxBufWrPtr++;
